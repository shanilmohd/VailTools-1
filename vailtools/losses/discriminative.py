"""
Implementation of the loss function described in 'Semantic Instance Segmentation
...
# tf.unique does not have a defined gradient
# This makes the default gradient an appropriate zeros tensor
tf.no_gradient("Unique")


def discriminative_loss(y_true, y_pred, batch_size=32):
    """
...
def var_loss(y_true, y_pred, centroids, delta=0.5):
    """
    ...
    def loop_body(i, loss):
        ...
        loss_ = tf.norm(instance - centroids[i], axis=-1)
...
def dist_loss(centroids, delta=2.0):
    """
    ...
    # Mask the comparison of a centroid to itself, still (C * C, E)
    # Implemented by masking comparisons of exactly matching centroids
    masked = tf.boolean_mask(diff, tf.reduce_sum(tf.abs(diff), axis=-1) > 0)

    # Final loss is a scalar
    return tf.reduce_mean(tf.square(tf.nn.relu(2.0 * delta - tf.norm(masked, axis=-1))))
...
def compute_centroids(y_true, y_pred):
    ...
    return tf.math.unsorted_segment_sum(y_pred, idx, tf.size(y)) / tf.expand_dims(
        tf.cast(counts, tf.keras.backend.floatx()), axis=-1
    )
...
def safe_norm(x, ord_=2, axis=None, keepdims=False, eps=1e-7, name="safe_norm"):
    ...
    normed = tf.pow(inner + eps, 1.0 / ord_)
