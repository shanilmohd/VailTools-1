...
def test_sparse_1d():
    ...
    model.fit(np.random.random((32, 16, 16)), np.random.random((32, 16, 8)))


def test_sparse_2d():
    model = keras.models.Sequential([layers.Sparse2D(filters=8)])
    model.compile(optimizer="adam", loss="mse")
    model.fit(np.random.random((32, 16, 16, 16)), np.random.random((32, 16, 16, 8)))
...
def test_sparse_3d():
    ...
    model.compile(optimizer="adam", loss="mse")
    model.fit(
        np.random.random((32, 16, 16, 16, 16)), np.random.random((32, 16, 16, 16, 8))
    )
